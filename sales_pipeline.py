...
def write_summary_sheet(wb, master, last_raw_row, col_index, numeric_cols, group_col):
    ...
    ws["A2"] = f"Generated {datetime.now().strftime('%d-%b-%Y %H:%M')} UTC  |  {last_raw_row - 1} rows"
    ...
    for r_idx, g in enumerate(groups, start=start_row + 1):
        row_fill = ZEBRA_FILL if (r_idx - start_row) % 2 == 0 else None
    ...
    top_border = Border(top=Side(style="thin", color="1F4E78"))
    ...
    if numeric_cols:
        ...
        # Dynamic "below average" highlight -- recalculates automatically if
        # the underlying data ever changes, instead of a fixed Python-computed
        # threshold baked in at generation time.
        ws.conditional_formatting.add(
            data_range,
            Rule(type="aboveAverage", aboveAverage=False, dxf=DifferentialStyle(
                fill=PatternFill(start_color="FFC7CE", end_color="FFC7CE", fill_type="solid")))
        )

        # Chart uses only groups with a genuinely non-zero total for the
        # primary metric -- a group with nothing to show is just clutter.
        ...
        group_row_map = {g: start_row + 1 + i for i, g in enumerate(groups)}

        if chart_groups:
            CHART_HELPER_COL = len(numeric_cols) + 4
            for i, g in enumerate(chart_groups, start=1):
                src_row = group_row_map[g]
                ws.cell(row=i, column=CHART_HELPER_COL, value=g)
                src_letter = get_column_letter(2)
                ws.cell(row=i, column=CHART_HELPER_COL + 1,
                        value=f"={src_letter}{src_row}")
            ws.column_dimensions[get_column_letter(CHART_HELPER_COL)].hidden = True
            ws.column_dimensions[get_column_letter(CHART_HELPER_COL + 1)].hidden = True

            chart = BarChart()
            chart.title = f"{numeric_cols[0]} by {group_col}"
            chart.y_axis.title = numeric_cols[0]
            chart.x_axis.title = group_col
            data_ref = Reference(ws, min_col=CHART_HELPER_COL + 1, min_row=1, max_row=len(chart_groups))
            cats_ref = Reference(ws, min_col=CHART_HELPER_COL, min_row=1, max_row=len(chart_groups))
...
def write_dashboard_sheet(wb, master, last_raw_row, col_index, numeric_cols, group_col, date_col, category_cols, status_col=None):
    ...
    ts_cell = ws.cell(row=3, column=1,
                       value=f"Generated {datetime.now().strftime('%d-%b %H:%M')} UTC")
    ts_cell.font = Font(name="Arial", size=7, color=LIGHT_GREY)
    ...
    if group_col and cat_start_row:
        ...
        if donut_n <= 8:
            ...
            if status_col and status_start_row:
                ...
                donut.dataLabels.showPercent = False
    ...
    if secondary_col and top_start_row:
        ...
        if len(top_items) <= 8:
            ...
            top_bar.dataLabels.showVal = True
    ...
    if group_col and cat_start_row and len(numeric_cols) >= 2:
        ...
        if len(chart_groups) <= 8:
            ...
            stacked.dataLabels.showPercent = True
    ...
    if group_col and cat_start_row and len(numeric_cols) >= 2:
        ...
        if show_combo_labels:
            ...
            combo_bar.dataLabels.showVal = True
        ...
        if show_combo_labels:
            ...
            combo_line.dataLabels.showVal = True
