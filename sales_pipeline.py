...
def write_dashboard_sheet(wb, master, last_raw_row, col_index, numeric_cols, group_col, date_col, category_cols, status_col=None):
    ...
    # ==================================================================
    CAT_COL = 30      # group_col breakdown (all numeric metrics)
    STATUS_COL_H = 45  # status_col breakdown (metric[0] only)
    TOPN_COL = 60      # secondary_col breakdown (metric[0] only)
    DATE_COL_H = 66    # date breakdown (metric[0] only)
    PCT_COL = 72       # achievement % derived from CAT_COL
    ...
    if group_col and groups:
        ...
        for r_idx, g in enumerate(groups, start=cat_start_row + 1):
        ...
        cat_end_row = cat_start_row + len(groups)
    ...
    if status_col and statuses:
        ...
        for r_idx, s in enumerate(statuses, start=status_start_row + 1):
        ...
        status_end_row = status_start_row + len(statuses)
    ...
    if group_col and cat_start_row:
        ...
        if status_col and status_start_row:
            ...
            donut_data_ref = Reference(ws, min_col=CAT_COL + 1, min_row=cat_start_row, max_row=cat_end_row)
        ...
        bar_data_ref = Reference(ws, min_col=CAT_COL + 1, min_row=cat_start_row, max_row=cat_end_row)
    ...
    if secondary_col and top_start_row:
        ...
        top_data_ref = Reference(ws, min_col=TOPN_COL + 1, min_row=top_start_row, max_row=top_end_row)
