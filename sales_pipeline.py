...
def write_dashboard_sheet(wb, master, last_raw_row, col_index, numeric_cols, group_col, date_col):
    """
    Builds a dedicated 'Dashboard' sheet that is genuinely interactive inside
    Excel/Google Sheets (no macros needed):
      - A dropdown cell lets you pick one category (e.g. one Dealer/Region)
        or "All".
      - The KPI cards and a small comparison chart are driven by live
        IF/SUMIFS formulas keyed off that dropdown, referencing 'Raw Data'
        directly -- so changing the dropdown instantly updates the numbers
        and the chart, with zero manual work.
      - Below that, 4 static overview charts (bar, pie, comparison, trend)
        give the full-picture view across everything.
    Note: openpyxl cannot create native Excel PivotTables/Slicers, so this
    dropdown+formula approach is the interactive mechanism used instead.
    """
    ...
    if group_col and groups:
        ws["A4"] = f"Filter by {group_col}:"
        ws["A4"].font = LABEL_FONT
        filter_cell = ws.cell(row=4, column=2, value="All")
        filter_cell.font = Font(name="Arial", bold=True, size=12, color="C00000")
        filter_cell.fill = PatternFill(start_color="FFF2CC", end_color="FFF2CC", fill_type="solid")
        filter_cell.border = BORDER
        filter_value_cell = "$B$4"

        # dropdown source list: "All" + every distinct group value
        list_col = 26  # column Z -- hidden helper list for the dropdown
        ...
        ws["D4"] = "<- Click this cell, use the dropdown arrow to pick one, or type 'All'"
        ws["D4"].font = Font(name="Arial", italic=True, size=9, color="666666")

    # ==================================================================
    # KPI CARDS -- live formulas, filtered by the dropdown when present
    # ==================================================================
    kpi_row = 6
    col_cursor = 1
    ...
    for metric in numeric_cols[:4]:
        metric_letter = get_column_letter(col_index[metric])
        label = f"Total {metric}" if not filter_value_cell else f"{metric} (selected)"
        ws.cell(row=kpi_row, column=col_cursor, value=label).font = LABEL_FONT
        if filter_value_cell and raw_group_letter:
            formula = (
        ...
        c = ws.cell(row=kpi_row, column=col_cursor + 1, value=formula)
        c.number_format = "#,##0"
        c.font = Font(name="Arial", bold=True, size=14, color="1F4E78")
        col_cursor += 3

    # ==================================================================
    # LIVE MINI CHART -- updates automatically when the dropdown changes
    # ==================================================================
    next_anchor_row = 9
    if filter_value_cell and raw_group_letter:
        mini_col = 26  # reuse column Z area, a few rows below the dropdown list
        mini_header_row = 3 + len(groups) + 3
        ws.cell(row=mini_header_row, column=mini_col, value="Metric")
        ws.cell(row=mini_header_row, column=mini_col + 1, value="Selected Value")
        for i, metric in enumerate(numeric_cols[:4], start=1):
            metric_letter = get_column_letter(col_index[metric])
            formula = (
                f'=IF({filter_value_cell}="All",'
                f"SUM('Raw Data'!${metric_letter}$2:${metric_letter}${last_raw_row}),"
            ...
            ws.cell(row=mini_header_row + i, column=mini_col, value=metric)
            ws.cell(row=mini_header_row + i, column=mini_col + 1, value=formula)
        mini_end_row = mini_header_row + min(len(numeric_cols), 4)
        ws.column_dimensions[get_column_letter(mini_col)].hidden = True
        ws.column_dimensions[get_column_letter(mini_col + 1)].hidden = True

        mini_chart = BarChart()
        mini_chart.visible_cells_only = False
        mini_chart.title = f"Selected {group_col}: Live Metrics (change the dropdown above)"
        mini_data_ref = Reference(ws, min_col=mini_col + 1, min_row=mini_header_row, max_row=mini_end_row)
        mini_cats_ref = Reference(ws, min_col=mini_col, min_row=mini_header_row + 1, max_row=mini_end_row)
        mini_chart.add_data(mini_data_ref, titles_from_data=True)
        mini_chart.set_categories(mini_cats_ref)
        mini_chart.width, mini_chart.height = 16, 8
        ws.add_chart(mini_chart, f"A{next_anchor_row}")
        next_anchor_row += 17

    # ==================================================================
    # STATIC HELPER DATA for the 4 overview charts below
    # ==================================================================
    helper_col = 20  # column T
    cat_start_row = None
    date_start_row = None
    ...
    # ==================================================================
    # 4 OVERVIEW CHARTS (stacked down column A, below the interactive part)
    # ==================================================================
    anchor_row = next_anchor_row
    ws.cell(row=anchor_row - 2, column=1, value="Full Overview (all data, unfiltered)").font = LABEL_FONT
    anchor_row += 1
