...
def write_summary_sheet(wb, master, last_raw_row, col_index, numeric_cols, group_col):
    ...
    n_cols = len(numeric_cols) + 1  # group column + one per metric

    start_row = 7
    THICK = Side(style="medium", color="1F4E78")
    ZEBRA_FILL = PatternFill(start_color="EAF1F8", end_color="EAF1F8", fill_type="solid")
    TOTAL_FILL = PatternFill(start_color="D9E4F0", end_color="D9E4F0", fill_type="solid")
    ...
    for r_idx, g in enumerate(groups, start=start_row + 1):
        zebra = (r_idx - start_row) % 2 == 0
        name_cell = ws.cell(row=r_idx, column=1, value=g)
        name_cell.font = LABEL_FONT
        name_cell.border = BORDER
        name_cell.alignment = Alignment(horizontal="left", vertical="center")
        if zebra:
            name_cell.fill = ZEBRA_FILL
    ...
    total_label.font = Font(name="Arial", bold=True)
    total_label.fill = TOTAL_FILL
    total_label.alignment = Alignment(horizontal="left", vertical="center")
    total_label.border = Border(left=THIN, right=THIN, top=THICK, bottom=THICK)
    ...
    # Outline the whole table with a slightly heavier border on the outside edge
    for col_idx in range(1, n_cols + 1):
        ws.cell(row=start_row, column=col_idx).border = Border(
            left=THICK if col_idx == 1 else THIN, right=THICK if col_idx == n_cols else THIN,
            top=THICK, bottom=THIN)
    for r in range(start_row + 1, total_row):
        left_cell = ws.cell(row=r, column=1)
        left_cell.border = Border(left=THICK, right=left_cell.border.right,
                                   top=left_cell.border.top, bottom=left_cell.border.bottom)
        right_cell = ws.cell(row=r, column=n_cols)
        right_cell.border = Border(left=right_cell.border.left, right=THICK,
    ...
    # Real auto-fit: size each column from the longest rendered value it
    # actually holds (header, group names, or formatted numbers), instead of
    # a single fixed width that either wastes space or clips long labels.
    for col_idx in range(1, n_cols + 1):
        col_letter = get_column_letter(col_idx)
        best = len(str(ws.cell(row=start_row, column=col_idx).value or ""))
        if col_idx == 1:
            for g in groups:
                best = max(best, len(str(g)))
            best = max(best, len("Total"))
        else:
            metric = numeric_cols[col_idx - 2]
            series = pd.to_numeric(master[metric], errors="coerce")
            grand_total = series.sum() if pd.notna(series.sum()) else 0
            best = max(best, len(f"{grand_total:,.0f}"))
        ws.column_dimensions[col_letter].width = min(max(best + 4, 12), 40)
...
def write_dashboard_sheet(wb, master, last_raw_row, col_index, numeric_cols, group_col, date_col, category_cols, status_col=None):
    ...
    def cond_expr(f):
        ...
        if f["kind"] == "month":
            return f'(({f["cell"]}="All")+(TEXT({rng},"mmm-yyyy")={f["cell"]})>0)'
        return f'(({f["cell"]}="All")+({rng}={f["cell"]})>0)'
    ...
    if group_col and chart_groups:
        ...
        ws.cell(row=2, column=BAR_PCT_COL, value=metric_formula(numeric_cols[0], exclude_keys={group_col}))
    ...
    if group_col and cat_start_row:
        ...
        donut.width, donut.height = 10, 8
        donut.legend.position = "b"
        donut.legend.overlay = False
        donut.dataLabels = DataLabelList()
        clean_labels(donut.dataLabels, showCatName=True, showPercent=True)
        donut.dataLabels.numFmt = "0.00%"
        donut.dataLabels.separator = "\n"
        donut.dataLabels.showLeaderLines = True
        ws.add_chart(donut, f"A{row1_anchor}")

        # Plain visible breakdown table (Category / Percentage) next to the
        # pie. Chart-level data labels above are a nice-to-have, but Google
        # Sheets silently drops custom pie data labels when it imports an
        # xlsx, so this ordinary cell-based table is the only thing that's
        # GUARANTEED to show every slice's percentage in every app.
        pct_hdr_row = row1_anchor
        ws.cell(row=pct_hdr_row, column=7, value=(status_col if (status_col and status_start_row) else group_col)).font = Font(name="Arial", bold=True, size=9, color=WHITE)
        ws.cell(row=pct_hdr_row, column=8, value="%").font = Font(name="Arial", bold=True, size=9, color=WHITE)
        for c in (7, 8):
            ws.cell(row=pct_hdr_row, column=c).fill = PatternFill("solid", fgColor=SECTION_BG)
        if status_col and status_start_row:
            pct_rows = zip(range(status_start_row + 1, status_end_row + 1), range(pct_hdr_row + 1, pct_hdr_row + 1 + len(chart_statuses)))
            src_col_letter = get_column_letter(STATUS_COL_H)
            val_col_letter = get_column_letter(STATUS_COL_H + 1)
        else:
            pct_rows = zip(range(cat_start_row + 1, cat_end_row + 1), range(pct_hdr_row + 1, pct_hdr_row + 1 + len(chart_groups)))
            src_col_letter = get_column_letter(CAT_COL)
            val_col_letter = get_column_letter(BAR_PCT_COL)
        for src_r, dst_r in pct_rows:
            ws.cell(row=dst_r, column=7, value=f"={src_col_letter}{src_r}").font = Font(name="Arial", size=9, color=WHITE)
            if status_col and status_start_row:
                ws.cell(row=dst_r, column=8, value=f"={val_col_letter}{src_r}/{grand_total_cell}")
            else:
                ws.cell(row=dst_r, column=8, value=f"={val_col_letter}{src_r}")
            ws.cell(row=dst_r, column=8).number_format = "0.00%"
            ws.cell(row=dst_r, column=8).font = Font(name="Arial", size=9, color=WHITE)
    ...
    if secondary_col and top_start_row:
        ...
        top_bar.y_axis.tickLblSkip = 1
        top_bar.width, top_bar.height = 15, 9
        top_bar.legend = None
        if len(top_items) <= 8:
    ...
    if group_col and cat_start_row and len(numeric_cols) >= 2:
        ...
        combo_bar.x_axis.tickLblSkip = 1
