...
def classify_columns(df):
    ...
    for col in df.columns:
        ...
        if numeric.notna().mean() > 0.6:
            # Guard against ID-like numeric columns that don't have an
            # obvious name (e.g. a bare "Code" column of near-unique
            # numbers): if almost every value is distinct, it's very
            # unlikely to be a meaningful quantity to sum/average.
            n_valid = numeric.notna().sum()
            if n_valid > 5 and (numeric.nunique(dropna=True) / n_valid) > 0.95:
                category_cols.append(col)
            else:
...
def build_master_dataset(local_dir):
    ...
    all_numeric.sort(key=score_numeric_column, reverse=True)
...
def write_dashboard_sheet(wb, master, last_raw_row, col_index, numeric_cols, group_col, date_col, category_cols, status_col=None):
    ...
    # ==================================================================
    # STATUS BREAKDOWN HELPER DATA (Payment Received / Pending / etc.)
    # -- far more meaningful than a generic category donut when the data
    # has an obvious outcome/status column, e.g. billing & collections data
    # ==================================================================
    status_start_row = None
    if status_col:
        status_agg = master.groupby(status_col)[numeric_cols[0]].sum(numeric_only=True).reset_index()
        status_agg = status_agg.sort_values(numeric_cols[0], ascending=False)
        helper_status_col = 50
        status_start_row = 3
        ws.cell(row=status_start_row, column=helper_status_col, value=status_col)
        ws.cell(row=status_start_row, column=helper_status_col + 1, value=numeric_cols[0])
        for r_idx, rec in enumerate(status_agg.itertuples(index=False), start=status_start_row + 1):
            ws.cell(row=r_idx, column=helper_status_col, value=rec[0])
            ws.cell(row=r_idx, column=helper_status_col + 1, value=rec[1])
        status_end_row = status_start_row + len(status_agg)
        ws.column_dimensions[get_column_letter(helper_status_col)].hidden = True
        ws.column_dimensions[get_column_letter(helper_status_col + 1)].hidden = True

    # ==================================================================
    # ROW 1 OF CHARTS: donut (status if available, else category) + bar
    # ==================================================================
    row1_anchor = 10
    if group_col and cat_start_row:
        ...
        if status_col and status_start_row:
            section_header(row1_anchor - 1, 1, 8, f"{numeric_cols[0]} by {status_col}")
            donut_cats_ref = Reference(ws, min_col=helper_status_col, min_row=status_start_row + 1, max_row=status_end_row)
            donut_data_ref = Reference(ws, min_col=helper_status_col + 1, min_row=status_start_row, max_row=status_end_row)
        else:
